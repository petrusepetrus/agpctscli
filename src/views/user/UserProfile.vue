...
async function getUserToBeEdited(userID) {
    ...
    try {
        ...
        if (response.data) {
            ...
            errorMessage.title = "No User Found"
            errorMessage.description = "We could not find a user with the ID of " + userID + ". If you think this is a mistake please contact support."
        }
    } catch (e) {
        userFound.value = false
        errorMessage = await errorMessageHandler(e)
        if (errorMessage.status === 401 || errorMessage.status === 419) {
